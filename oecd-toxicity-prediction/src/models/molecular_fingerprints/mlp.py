...
def main(fingerprint_type, file_path, model_save_path):
    ...
    for p in tqdm(range(len(params))):
        ...
        for train_idx, val_idx in kf.split(x_train, y_train):
            ...
            model = MLPClassifier(random_state=42, **params[p])
        ...
        # 중간 결과 저장
        save_results(result, f'{model_save_path}/mlp_intermediate_{fingerprint_type}.json')
    ...
    final_model = MLPClassifier(random_state=42, **best_model)
    ...
    # 최적 모델 저장
    model_filename = f'{model_save_path}/best_model_{fingerprint_type}_mlp.joblib'
