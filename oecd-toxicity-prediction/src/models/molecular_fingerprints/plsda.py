...
def main(fingerprint_type, file_path, model_save_path):
    ...
    for p in tqdm(range(len(params))):
        ...
        for train_idx, val_idx in kf.split(x_train, y_train):
            ...
            pred = [1 if x >= 0.5 else 0 for x in model.predict(fold_val_x)[:, 0]]
        ...
        # 중간 결과 저장
        save_results(result, f'{model_save_path}/plsda_intermediate_{fingerprint_type}.json')
    ...
    final_model = PLSRegression(**best_model)
    final_model.fit(x_train, y_train)
    final_pred_probs = final_model.predict_proba(x_test)[:, 1]  # 확률 예측값 사용
    final_pred = [1 if x >= 0.5 else 0 for x in final_model.predict(x_test)[:, 0]]
    ...
    # 최적 모델 저장
    model_filename = f'{model_save_path}/best_model_{fingerprint_type}_plsda.joblib'
...
if __name__ == '__main__':
    ...
    main(args.fingerprint_type, args.file_path, args.model_save_path)
