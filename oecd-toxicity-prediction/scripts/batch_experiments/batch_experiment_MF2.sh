...
if [ "$EXPERIMENT_TYPE" == "MF" ]; then
    ...
    file_paths=('../data/TG201.xlsx') # '../data/TG202.xlsx' '../data/TG203.xlsx' '../data/TG210.xlsx' '../data/TG211.xlsx' 
    model_subdir="molecular_fingerprints"
    use_scaler=false
elif [ "$EXPERIMENT_TYPE" == "MD" ]; then
    # Molecular Descriptors 데이터 (스케일러 필요)
    file_paths=('../data/TG+MD/TG201_Descriptor_desalt.xlsx') # Add other MD files
    model_subdir="molecular_descriptors"
    use_scaler=true
elif [ "$EXPERIMENT_TYPE" == "COMBINED" ]; then
    # Combined MF+MD 데이터 (스케일러 필요)
    file_paths=('../../data/TG+MD/TG414_Descriptor_desalt_250501.xlsx' '../../data/TG+MD/TG416_Descriptor_desalt_250501.xlsx')
